<template>
  <div id="detail">
    <section class="user-info">
      ...
      <h3>前端异步大揭秘</h3>
      <p>
        <router-link to="/user">若愚</router-link> 发布于3天前</p>
    </section>
    <section class="article">

      <h1 id="css-网格布局学习指南">CSS 网格布局学习指南</h1>
      <p>CSS 网格布局(Grid Layout) 是CSS中最强大的布局系统。 这是一个二维系统，这意味着它可以同时处理列和行，不像
        <a href="https://css-tricks.com/snippets/css/a-guide-to-flexbox/" target="_blank">flexbox</a> 那样主要是一维系统。 你可以通过将CSS规则应用于父元素（成为网格容器）和该元素的子元素（网格元素），来使用网格布局。</p>
      <blockquote>
        <p>
          <a href="https://css-tricks.com/snippets/css/complete-guide-grid/" target="_blank">本文</a>出自于
          <a href="http://chris.house/blog/a-complete-guide-css-grid-layout/" target="_blank">Chris House</a> 的个人博客，由
          <a href="https://jirengu.com" target="_blank">饥人谷</a>
          <a href="https://www.zhihu.com/people/jirengu-ruo-yu" target="_blank">若愚老师</a>翻译</p>
      </blockquote>
      <h2 id="引言">引言</h2>
      <p>CSS网格布局（又名“网格”）是一个二维的基于网格的布局系统，其目的只在于完全改变我们设计基于网格的用户界面的方式。 CSS一直用来布局网页，但一直都不完美。 一开始我们使用table 做布局，然后转向浮动、定位以及inline-block，但所有这些方法本质上都是 Hack 的方式，并且遗漏了很多重要的功能（例如垂直居中）。 Flexbox的出现在一定程度上解决了这个问题，但是它的目的是为了更简单的一维布局，而不是复杂的二维布局（Flexbox和Grid实际上一起工作得很好）。 只要我们一直在制作网站，我们就一直在为解决布局问题不断探索， 而Grid是第一个专门为解决布局问题而生的CSS模块。
      </p>
      <p>有两个东西，启发我写这篇指南。 第一个是雷切尔·安德鲁（Rachel Andrew）的书
        <a href="http://abookapart.com/products/get-ready-for-css-grid-layout" target="_blank">为CSS Grid布局准备</a>。 这本书对网格布局做了彻底、清晰的介绍，也是是整篇文章的基础，我强烈建议你购买并阅读他的书。 我的另一个重要灵感是Chris Coyier的
        <a href="https://css-tricks.com/snippets/css/a-guide-to-flexbox/" target="_blank">Flexbox完全指南</a>，当需要查阅 flexbox 的一切资料时我就会找这篇文章。 这篇文章帮助了很多人学习 Flex 布局，也是 Google 上搜索“flexbox”关键字排名第一的文章。你会发现他的文章和我的很多相似之处，有最好的范例在那放着为什么咱不偷师学着写呢？</p>
      <p>本指南的目的是介绍网格概念，因为它们存在于最新版本的规范中。 因此我不会覆盖过时的IE语法，而且随着规范的成熟，我会尽最大努力保存更新本指南。</p>
      <h2 id="基础知识以及浏览器支持情况">基础知识以及浏览器支持情况</h2>
      <p>一开始你需要使用
        <code>display：grid</code>把容器元素定义为一个网格，使用
        <code>grid-template-columns</code>和
        <code>grid-template-rows</code>设置列和行大小，然后使用
        <code>grid-column</code> 和
        <code>grid-row</code>把它的子元素放入网格。 与flexbox类似，网格子元素的原始顺序不重要。 你的可以在 CSS 里以任意顺序放置它们，这使得使用媒体查询重新排列网格变得非常容易。 想象一下，我们需要定义整个页面的布局，然后为了适应不同的屏幕宽度完全重新排列，我们只需要几行CSS就能实现这个需求。 网格是有史以来最强大的CSS模块之一。</p>
      ...
      <blockquote>
        <p>浏览器支持的详细数据可在
          <a href="http://caniuse.com/#feat=css-grid" target="_blank">Caniuse</a>查看。其中里面的数字表示该版本以上的浏览器支持Grid。</p>
      </blockquote>
      ...
      <p>设置了
        <code>display: gird</code> 的元素。 这是所有 grid item 的直接父项。 在下面的例子中，
        <code>.container</code> 就是是 grid container。</p>
...
<span class="hljs-tag">&lt;/<span class="hljs-name">div</span>&gt;</span>
      ...
      <p>Grid 容器的孩子（直接子元素）。下面的
        <code>.item</code> 元素就是 grid item，但
        <code>.sub-item</code>不是。</p>
      <pre><code class="lang-html"><span class="hljs-tag">&lt;<span class="hljs-name">div</span> <span class="hljs-attr">class</span>=<span class="hljs-string">"container"</span>&gt;</span>
...
<span class="hljs-tag">&lt;/<span class="hljs-name">div</span>&gt;</span>
      ...
      <ul>
        <li>
          <strong>grid</strong> - 生成一个块级(block-level)网格</li>
        <li>
          <strong>inline-grid</strong> - 生成一个行级(inline-level)网格</li>
        <li>
          <strong>subgrid</strong> - 如果你的 grid container 本身就是一个 grid item（即,嵌套网格），你可以使用这个属性来表示你想从它的父节点获取它的行/列的大小，而不是指定它自己的大小。</li>
      </ul>
  ...
  <span class="hljs-attribute">display</span>: grid | inline-grid | subgrid;
      ...
      <p>注意：
        ...
        <code>float</code>,
        <code>clear</code>, 以及
        <code>vertical-align</code> 对一个 grid container 没有影响</p>
  ...
  <span class="hljs-attribute">grid-template-rows</span>: [row1-start] <span class="hljs-number">25%</span> [row1-end row2-start] <span class="hljs-number">25%</span> [row2-end];
      ...
      <p>如果你的定义中包含重复的部分，则可以使用
        <code>repeat()</code> 符号来简化写法：</p>
      <pre><code class="lang-css"><span class="hljs-selector-class">.container</span> {
  ...
  <span class="hljs-attribute">grid-template-columns</span>: <span class="hljs-number">1</span>fr <span class="hljs-number">50px</span> <span class="hljs-number">1</span>fr <span class="hljs-number">1</span>fr;
      ...
      <p>通过引用
        <code>grid-area</code>属性指定的网格区域的名称来定义网格模板。 重复网格区域的名称导致内容扩展到这些单元格。 点号表示一个空单元格。 语法本身提供了网格结构的可视化。</p>
      <p>值：</p>
      <ul>
        <li>
          <code>&lt;grid-area-name&gt;</code> - 使用 grid-area 属性设置的网格区域的名称</li>
        <li>
          <code>.</code> - 点号代表一个空网格单元</li>
        <li>
          <strong>none</strong> - 没有定义网格区域</li>
      </ul>
      <p>举例：</p>
...
<span class="hljs-selector-class">.container</span> {
  ...
  <span class="hljs-attribute">grid-template-areas</span>: 
    ...
    <span class="hljs-string">"footer footer footer footer"</span>;
      ...
      <p>这将创建一个四列宽三行高的网格。 整个第一行将由
        <strong>header</strong> 区域组成。 中间一行将由两个
        <strong>main</strong> 区域、一个空单元格和一个
        <strong>sidebar</strong> 区域组成。 最后一行是
        <strong>footer</strong>区域组成。</p>
      <p><img src="https://cloud.hunger-valley.com/18-1-11/45883436.jpg" alt=""></p>
      <p>你的声明中的每一行都需要有相同数量的单元格。</p>
      <p>您可以使用任意数量的相邻的
        <code>.</code>来声明单个空单元格。 只要这些点号之间没有空格，他们就代表了一个单一的单元格。</p>
      <p>需要注意的是你不是在用这个语法命名网格线，而是在命名区域。 当你使用这种语法时，区域两端的网格线实际上是自动命名的。 比如，如果网格区域的名称是foo，那么区域的起始的行网格线和列网格线名称是 foo-
        <strong>start</strong>，并且区域终点的行网格线和列网格线名称是 foo-
        <strong>end</strong>。 这意味着某些网格线可能有多个名称，比如上面的例子中最左边的一条网格线有三个名字：header-start，main-start 和 footer-start。</p>
      <h3 id="grid-template">grid-template</h3>
      <p>在单个声明中定义 grid-template-rows、grid-template-columns、grid-template-areas 的简写。</p>
      <p>值：</p>
      <ul>
        <li>
          <strong>none</strong> - 将三个属性都设置为其初始值</li>
        <li>
          <strong>subgrid</strong> - 把 grid-template-rows 和 grid-template-columns 设置为 subgrid, 并且 grid-template-areas 设置为初始值</li>
        <li>
          <code>grid-template-rows</code> /
          <code>&lt;grid-template-columns</code> - 把 grid-template-columns 和 grid-template-rows 设置为指定值, 与此同时, 设置 grid-template-areas 为 none</li>
      </ul>
      <pre><code class="lang-css"><span class="hljs-selector-class">.container</span> {
  ...
  <span class="hljs-attribute">grid-template-areas</span>: 
    ...
    <span class="hljs-string">"footer footer footer"</span>;
      ...
      <p>由于 grid-template 不会重置隐式网格属性（
        <code>grid-auto-columns</code>，
        <code>grid-auto-rows</code>和
        <code>grid-auto-flow</code>），而这可能是大多数情况下你想要做的。因此建议使用
        <code>grid</code>属性来代替
        <code>grid-template</code>。</p>
      <h3 id="grid-column-gap--grid-row-gap">grid-column-gap / grid-row-gap</h3>
      <p>指定网格线的大小，你可以把它想象为设置列/行之间的间距的宽度。</p>
      <p>值：</p>
      <ul>
        <li>
          <code>line-size</code> - 一个长度值</li>
      </ul>
      <pre><code class="lang-css"><span class="hljs-selector-class">.container</span> {
  ...
  <span class="hljs-attribute">grid-gap</span>: <span class="hljs-number">10px</span> <span class="hljs-number">15px</span>;
      ...
      <ul>
        <li>
          <strong>start</strong>: 内容与网格区域的左端对齐</li>
        <li>
          <strong>end</strong>: 内容与网格区域的右端对齐</li>
        <li>
          <strong>center</strong>: 内容位于网格区域的中间位置</li>
        <li>
          <strong>stretch</strong>: 内容宽度占据整个网格区域空间(这是默认值)</li>
      </ul>
      <pre><code class="lang-css"><span class="hljs-selector-class">.container</span> {
  ...
  <span class="hljs-attribute">justify-items</span>: stretch;
      ...
      <p>也可以通过给单个 grid item 设置
        <code>justify-self</code>属性来达到上述效果。</p>
      <h3 id="align-items">align-items</h3>
      <p>沿着列轴对齐grid item 里的内容（与之对应的是使用 justify-items 设置沿着行轴对齐），该值适用于容器内的所有 grid items。</p>
      <p>值：</p>
      <ul>
        <li>
          <strong>start</strong>: 内容与网格区域的顶端对齐</li>
        <li>
          <strong>end</strong>: 内容与网格区域的底部对齐</li>
        <li>
          <strong>center</strong>: 内容位于网格区域的垂直中心位置</li>
        <li>
          <strong>stretch</strong>: 内容高度占据整个网格区域空间(这是默认值)</li>
      </ul>
      <pre><code class="lang-css"><span class="hljs-selector-class">.container</span> {
  ...
  <span class="hljs-attribute">align-items</span>: stretch;
      ...
      <p>也可以通过给单个 grid item 设置
        <code>align-self</code>属性来达到上述效果。</p>
  ...
  <span class="hljs-attribute">align-content</span>: space-evenly;  
      ...
      <ul>
        <li>
          <code>&lt;track-size&gt;</code> - 可以是一个长度值，一个百分比值，或者一个自由空间的一部分（使用 fr 单位）</li>
      </ul>
      <pre><code class="lang-css"><span class="hljs-selector-class">.container</span> {
...
<span class="hljs-selector-class">.item-b</span> {
  ...
  <span class="hljs-attribute">grid-auto-flow</span>: row | column | row dense | column dense
      ...
      <p>需要注意的是，
        <code>dense</code> 可能导致您的 grid item 乱序。</p>
      <p>举例， 考虑如下 HTML：</p>
...
<span class="hljs-selector-class">.item-e</span> {
  ...
  <span class="hljs-attribute">grid-auto-flow</span>: column;
      ...
      <ul>
        ...
        <li>
          <code>&lt;grid-template-rows&gt;</code> /
          <code>&lt;grid-template-columns&gt;</code> - 将 grid-template-rows 和 grid-template-columns 分别设置为指定值，将所有其他子属性设置为其初始值</li>
        <li>
          <code>&lt;grid-auto-flow&gt;</code> [
          <code>&lt;grid-auto-rows&gt;</code> [ /
          <code>&lt;grid-auto-columns&gt;</code>] ] - 接受所有与grid-auto-flow，grid-auto-rows和grid-auto-columns相同的值。 如果省略grid-auto-columns，则将其设置为为grid-auto-rows指定的值。 如果两者都被省略，则它们被设置为它们的初始值</li>
      </ul>
      <pre><code class="lang-css"><span class="hljs-selector-class">.container</span> {
...
<span class="hljs-selector-class">.container</span> {
  ...
  grid-row-end: span <span class="hljs-number">2</span>
      ...
      <ul>
        <li>
          <code>&lt;start-line&gt;</code> /
          <code>&lt;end-line&gt;</code> - 每个值的用法都和属性分开写时的用法一样</li>
      </ul>
      <pre><code class="lang-css"><span class="hljs-selector-class">.item</span> {
  ...
  <span class="hljs-attribute">grid-row</span>: third-line / <span class="hljs-number">4</span>;
      ...
      <ul>
        ...
        <li>
          ...
          <code>&lt;row-end&gt;</code> /
          <code>&lt;column-end&gt;</code> - 可以是数字，也可以是网格线的名字</li>
      </ul>
      <pre><code class="lang-css"><span class="hljs-selector-class">.item</span> {
